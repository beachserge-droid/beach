...
export default function AdminBlogCategoriesPage() {
  ...
  const updateCategory = async (id, patch) => {
    const res = await fetch(`/api/admin/blog/categories/${encodeURIComponent(String(id))}`, {
      method: "PUT",
      headers: { "content-type": "application/json" },
  ...
  return (
    <div>
      <div className="admin-toolbar">
        <div>
          <h1 className="admin-toolbar__title">Blog Kategorileri</h1>
          <div className="admin-muted" style={{ fontSize: 13, marginTop: 4 }}>
            Blog yazılarını gruplamak için kategorileri yönet.
          </div>
        </div>
      </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: 10, marginBottom: 12 }}>
                ...
                <div style={{ gridColumn: "span 6" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 8 }}>
                    <input className="admin-checkbox" type="checkbox" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} id="blogCatActiveNew" />
                    <label htmlFor="blogCatActiveNew" className="admin-muted" style={{ fontSize: 13 }}>Yayında</label>
                  </div>
                </div>
              </div>
        ...
        <div style={{ gridColumn: "span 7" }}>
          <div className="admin-table-wrap">
            <div style={{ padding: 14, borderBottom: "1px solid rgba(15, 17, 21, 0.08)", display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 12 }}>
              ...
              <div className="admin-muted" style={{ fontSize: 12 }}>Toplam: {categories.length}</div>
            </div>
            <div style={{ overflowX: "auto" }}>
              <table className="admin-table">
                ...
                <tbody>
                    ...
                    <tr key={c.id}>
                      ...
                      <td style={{ width: 110 }}>
                        <input
                          ...
                          onChange={(e) => setCategories((prev) => prev.map((x) => x.id === c.id ? { ...x, isActive: e.target.checked } : x))}
                          aria-label="Aktif"
                        />
                      </td>
                  ...
                  {categories.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="is-muted" style={{ padding: 18 }}>Henüz kategori yok.</td>
                    </tr>
                  ) : null}
                </tbody>
              </table>
